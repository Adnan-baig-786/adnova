...
const Connections = () => {
  ...
  const currentTabUsers = (dataArray.find((item) => item.label === currentTab)?.value || []).filter(Boolean)
      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
          {dataArray.map((tab) => (
            <button
              ...
              key={tab.label}
              className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black shadow-xs' : 'text-gray-500 hover:text-black'}`}
            >
              <tab.icon className='w-4 h-4'/>
              <span className='ml-1'>{tab.label}</span>
              <span className='ml-2 text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full'>{tab.value?.length || 0}</span>
            </button>
          ))}
        </div>
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {currentTabUsers.length > 0 ? (
            currentTabUsers.map((user) => (
              <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
                ...
                <div className='flex-1'>
                  ...
                  <p className="text-slate-500">@{user.username || 'user'}</p>
            ...
            <p className='text-gray-500 py-8'>No {currentTab.toLowerCase()} found.</p>
          )}
        </div>
      </div>
    </div>
